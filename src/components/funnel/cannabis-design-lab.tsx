...
function Pain({ concept }: { concept: ConceptId }) { return <section className={styles.pain}><div className={styles.painPhoto} aria-hidden="true"><span>Friday · 4:20 PM</span></div><div className={styles.painCopy}><span>The response problem</span><h2>{concept === "calm-control" ? "The difference your customers feel." : concept === "sale-in-call" ? "Every missed call is a sale walking out." : "Your phone is blowing up. Your team is helping customers."}</h2><p>When no one answers, shoppers do not wait. They call the next store. Sage handles routine questions while your team protects the experience inside.</p><div className={styles.compare}><article><b>{concept === "calm-control" ? "Interrupted counter" : "Without Sage"}</b><p>Calls pull staff off the floor</p><p>Questions wait or go unanswered</p><p>Promotions go unnoticed</p></article><article><b>{concept === "calm-control" ? "Protected counter" : "With Sage"}</b><p><Check /> Routine calls handled end to end</p><p><Check /> Accurate answers from store data</p><p><Check /> Every caller gets a next step</p></article></div></div></section>; }
